...
function CardSegmento({ seg }) {
  const estrutura = seg.estruturaEmpresa || [];
  return (
    <article className="segmentos__card" style={{ '--seg-cor': seg.cor }}>
      ...
      <div className="segmentos__card-body">
        {estrutura.length > 0 && (
          <section className="segmentos__bloco segmentos__estrutura">
            <h3 className="segmentos__bloco-titulo">Estrutura da empresa (diretoria → cliente)</h3>
            <ul className="segmentos__estrutura-lista">
              {estrutura.map((item, i) => (
                <li key={i} className="segmentos__estrutura-item">
                  <span className="segmentos__estrutura-nivel">{item.nivel}</span>
                  {item.descricao && (
                    <span className="segmentos__estrutura-desc">{item.descricao}</span>
                  )}
                </li>
              ))}
            </ul>
...
export default function PaginaSegmentos() {
  ...
  const segmentosFiltrados = useMemo(() => {
    ...
    const textEstrutura = (s) =>
      (s.estruturaEmpresa || [])
        .map((e) => `${e.nivel} ${e.descricao || ''}`)
        .join(' ')
        .toLowerCase();
